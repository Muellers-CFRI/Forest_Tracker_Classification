"""
---------------------------------------------------------------------------
Title: GEOTRACKS Duplicate Shape Identification and Dissolve Script

Purpose:
    This script identifies and resolves duplicate treatment
    features within the U.S. Forest Service GEOTRACKS dataset prior to Forest Tracker
...
Major Steps:
    1. Create a working copy of the GEOTRACKS reclassified perimeters.
...
Inputs:
    GEOTRACKS_reclass  – Perimeter feature class with reclassified GEOTRACKS treatments.
...
# Base workspace
base_fldr = r"E:\CFRI\FOREST_TRACKER\LOCAL_DATA\CFTv2_Data_Handoff"
arcpy.env.workspace = base_fldr

# Directories and files
GEOTRACKS_reclass = os.path.join(base_fldr, 'CFTv2_Data_Handoff.gdb/CFTv2_CSFS_GeoTracks_classified_v1')

# Scratch and output locations
scratch_gdb = os.path.join(base_fldr, "scratch.gdb")
...
# Create working copy
arcpy.CopyFeatures_management(GEOTRACKS_reclass, find_identical)
...
group_fields = ["DuplID", "ActClass", "COMPLETED1"]
...
agg_fields = [
    ["COMPLETED_", "MAX"],  # # OF UNITS
    ["UNIT_OF_ME", "FIRST"],  # UNITS OF MEASURE (acres, trees, seedlings, each...)
    ["NAME_REDACT", "FIRST"],
    ["ID_CODE", "FIRST"],
    ["FUNDING_SO", "CONCATENATE"],
    ["DESCRIPTION_REDACT", "CONCATENATE"],
    ["COMMENT_REDACT", "CONCATENATE"],
    ["Class_Combine", "CONCATENATE"]
]
...
for f in fields:
    for prefix in ("SUM_", "FIRST_", "CONCATENATE_", "MAX_"):
